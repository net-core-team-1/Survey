...
namespace Survey.Outbox.EntityFramwork
{
       ...
       where TContext : DbContext
    {
        ...
        async Task<IReadOnlyList<OutboxMessage>> IMessageOutboxAccessor.GetUnsentAsync()
        {
            ...
            {
                if (om.MessageContextType != null)
                {
                    var messageContextType = Type.GetType(om.MessageContextType);
                    om.MessageContext = ObjectToByteArray(JsonConvert.DeserializeObject(om.SerializedMessageContext, messageContextType,
                        SerializerSettings));
                }

                if (om.MessageType != null)
                {
                    var messageType = Type.GetType(om.MessageType);
                    om.Message = ObjectToByteArray(JsonConvert.DeserializeObject(om.SerializedMessage, messageType, SerializerSettings));
                }
