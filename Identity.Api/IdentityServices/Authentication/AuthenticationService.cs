...
namespace Identity.Api.IdentityServices.Authentication
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<Result> LoginAsync(string name, string password)
        {
        ...
        public async Task<Result> LogOut(Guid id)
        {
            var user = await _userManager.FindByIdAsync(id.ToString());
            if (user == null)
                return await Task<Result>.FromResult(Result.Failure($"Invalid_user_id"));
        ...
        public async Task<JsonWebTokenResponse> IssueWebToken(string id = "", string email = "")
        {
            ...
            //else
            user = await _userService.FindUserByUserNameAsync(id);
